...
|---|---|
| `deploy/Dockerfile` (modificar) | cria e dá dono aos **três** pontos de montagem |
...
Acrescente `import uuid` ao bloco de imports do topo do arquivo.
...
Esperado: seis linhas `OK:` e `Todos os testes da rota de saúde passaram.`
...
- [ ] **Passo 3: corrigir `deploy/Dockerfile` — os três pontos de montagem**

O Dockerfile existente cria e dá dono só a `/dados`. `/registros` e `/banco`
não existem na imagem, então o Docker os cria **como root** ao montar os
volumes — e o app roda como `servico`. Consequência no primeiro
`docker compose up -d`: `sqlite3.connect("/banco/contas.db")` falha,
`/api/saude` devolve 503 para sempre, o Caddy (que depende da saúde do app)
nunca sobe, e nada vai ao ar.

Troque a linha `RUN mkdir -p /dados && chown servico:servico /dados` por:
...
- [ ] **Passo 4: escrever `deploy/docker-compose.yml`**
...
services:
  app:
    ...
    # (Escrito assim, sem a palavra-chave literal: o teste do Passo 1 procura
    # a diretiva no bloco do `app`, e um comentário que a cite falharia.)
  ...
  caddy:
    ...
    ports:
      - "80:80"
      - "443:443"
    # Sem `env_file` de propósito: o `.env` inteiro traz o segredo de sessão e
    # a senha do SMTP, e o Caddy não usa nem um nem outro. Do arquivo ele
    # precisa de uma variável só, e é só ela que entra.
    environment:
      DOMINIO: ${DOMINIO}
    volumes:
...
- [ ] **Passo 5: escrever `deploy/Caddyfile`**
...
- [ ] **Passo 6: escrever `deploy/.env.exemplo`**
...
# --- backup -----------------------------------------------------------------
# O comando que leva a cópia para fora da VPS. `{arquivo}` marca onde entra o
# caminho; sem o marcador, ele vai para o fim. O marcador existe porque o
# rclone quer a origem ANTES do destino, e apendar no fim inverteria a cópia.
#   rclone: rclone copyto --config /etc/rclone.conf {arquivo} remoto:pdftodxf/contas.db
#   aws:    aws s3 cp {arquivo} s3://balde/pdftodxf/
...
```bash
git add deploy/Dockerfile deploy/docker-compose.yml deploy/Caddyfile deploy/.env.exemplo tests/test_deploy_config.py
...
`if __name__ == "__main__":`, e some as sete chamadas àquele bloco:
...
on:
  # **Depois do CI, e só se ele passou.** O Dockerfile roda apenas `npm test`;
  # a suíte Python inteira mora no workflow `CI`. Disparar no push à `main`,
  # como era antes, deixava um commit com teste Python quebrado construir,
  # publicar e ir ao ar enquanto o CI ficava vermelho em paralelo — o desenho
  # exige o contrário: falhou, não constrói.
  workflow_run:
    workflows: [CI]
    types: [completed]
    branches: [main]

  # Disparo manual. É assim que sai a **primeira** imagem: a VPS não compila
  # nada por desenho, então o `.env` do primeiro deploy precisa de uma etiqueta
  # que já exista no ghcr, e ela só pode vir daqui — antes de `DEPLOY_ATIVO`
  # existir. Também serve para republicar sem commit novo.
  workflow_dispatch:
...
env:
  # No evento `workflow_run`, `github.sha` é o topo da branch padrão, que pode
  # já ter andado. O commit que o CI aprovou é o `head_sha` do run — é dele que
  # a imagem tem de nascer, e é ele que vira etiqueta.
  COMMIT: ${{ github.event.workflow_run.head_sha || github.sha }}

jobs:
  imagem:
    name: Construir e publicar a imagem
    # O `imagem` roda **sem** DEPLOY_ATIVO quando disparado à mão: é o que
    # produz a primeira etiqueta, antes de a VPS existir. Só o job `publicar`,
    # que precisa dos segredos de ssh, exige a variável ligada.
    if: >-
      (github.event_name == 'workflow_dispatch'
       || github.event.workflow_run.conclusion == 'success')
      && (vars.DEPLOY_ATIVO == '1' || github.event_name == 'workflow_dispatch')
    ...
    steps:
      ...
      - id: nome
        run: |
          ...
          echo "etiqueta=ghcr.io/${repo}:${COMMIT}" >> "$GITHUB_OUTPUT"
      ...
      # Cinto e suspensório: o gatilho já garante que o CI passou, e o
      # Dockerfile roda `npm test` no primeiro estágio — uma imagem não nasce
      ...
      # A etiqueta na tela do run: no primeiro deploy ela é copiada à mão para
      # o `IMAGEM=` do `.env` da VPS, e caçá-la no log não é roteiro.
      - name: Anotar a etiqueta no resumo
        run: |
          {
            echo "### Imagem publicada"
            echo
            echo '```'
            echo "IMAGEM=${{ steps.nome.outputs.etiqueta }}"
            echo '```'
          } >> "$GITHUB_STEP_SUMMARY"
  ...
  publicar:
    ...
    needs: imagem
    # Sem DEPLOY_ATIVO não há VPS nem segredos de ssh: o disparo manual
    # constrói a imagem e para aqui, que é o que o primeiro deploy precisa.
...
> **Por que `workflow_run` sobre o `CI`, e não `push`.** O Dockerfile roda
> `npm test` e mais nada; a suíte Python inteira mora no `ci.yml`. Com gatilho
> de push, os dois workflows correm em paralelo e um commit com teste Python
> quebrado constrói, publica e vai ao ar enquanto o CI fica vermelho ao lado —
> o oposto do "falhou, não constrói" do desenho. `workflow_run` dispara também
> quando o CI **falha**, por isso a guarda `conclusion == 'success'` é
> obrigatória, não decorativa. E como no evento `workflow_run` o `github.sha` é
> o topo da branch padrão (que pode já ter andado), o checkout e a etiqueta
> usam `github.event.workflow_run.head_sha`: a imagem tem de nascer exatamente
> do commit que ficou verde.

> **Por que `workflow_dispatch`.** A VPS não compila nada, então o `IMAGEM=` do
> primeiro `.env` precisa de uma etiqueta que já exista no ghcr — e ela tem de
> sair **antes** de `DEPLOY_ATIVO`, que só se liga depois do serviço de pé. O
> disparo manual constrói e publica a imagem sem exigir a variável; o job
> `publicar`, esse sim, continua exigindo, e aparece pulado.
...
- Produz: `copiar(origem, destino) -> Path`,
  `apagar_antigos(pasta, dias, agora) -> list[Path]` e
  `dias_de_retencao() -> int`.
...
**3. A primeira imagem, construída à mão.** A VPS não compila nada — por
desenho, a imagem chega pronta. Então o `IMAGEM=` do primeiro `.env` precisa de
uma etiqueta que já exista no ghcr, e ela tem de sair **antes** de
`DEPLOY_ATIVO` (passo 6). É para isso que o workflow aceita disparo manual:

```bash
gh workflow run Deploy          # ou: Actions → Deploy → Run workflow
gh run watch
gh run view --log | grep 'IMAGEM='      # ou leia o resumo do run
```

O job `Trocar a imagem na VPS` aparece **pulado**: sem `DEPLOY_ATIVO` ele não
roda, e aqui só se quer a imagem.

**Torne o pacote público.** Pacote publicado com o `GITHUB_TOKEN` nasce
privado, **mesmo em repositório público** — sem este passo, o
`docker compose pull` da VPS leva `denied` e o passo 4 não sai do lugar. No
GitHub: Packages → `pdftodxf` → *Package settings* → *Change visibility* →
*Public*. Confira de fora:

```bash
docker logout ghcr.io && docker pull <etiqueta>
```

> Escolhemos pacote público em vez de `docker login ghcr.io` na VPS: um token
> de leitura na VPS é mais um segredo para guardar, rodar e vazar, e a imagem
> não contém segredo nenhum (o `.env` vive só na VPS).

**4. Primeiro deploy, manual.** O automático só entra no passo 6:
...
**5. Brevo.** Crie a conta, verifique o domínio publicando SPF, DKIM e DMARC no
...
**6. Ligar a publicação automática.** Nesta ordem — a variável por último:
...
o workflow `CI` ficar verde**, que é de quem o Deploy espera o sinal. Acompanhe
em Actions e confirme que a etiqueta nova está no ar:

```bash
grep '^IMAGEM=' /opt/pdftodxf/.env
```

**7. Backup no `cron`.** Diário, e a restauração testada uma vez. **No crontab
do `root`**: `/var/lib/docker/volumes/` não é atravessável por quem não é root,
e nem `/var/backups/pdftodxf` nem `/var/log/pdftodxf-backup.log` são criáveis
pelo usuário de deploy — no crontab dele, a tarefa falharia toda noite.
...
A linha do comando é **uma só**: o `cron` não entende continuação com `\`.
...
> `cron` lê `NOME=valor` literalmente, sem shell no meio. (No `cron`, `%` é
> especial: escape como `\%` se o comando de envio tiver um.)
...
**8. Sonda.** Cadastre `https://<dominio>/api/saude` no UptimeRobot (ou
...
- [ ] A integração contínua está verde, e o workflow de deploy aparece como
      **pulado** enquanto `DEPLOY_ATIVO` não existir — com a exceção do
      disparo manual, que constrói a imagem e para antes de tocar na VPS.
- [ ] O `.env` da VPS tem `IMAGEM=`, `DOMINIO=` e `PDFTODXF_SEGREDO=`
      preenchidos, e o pacote no ghcr está **público** (`docker pull` funciona
      deslogado).
- [ ] Um arquivo de 99 MiB — acima dos 100 MB do SI, abaixo do teto real —
      recebe a mensagem do app, não um 413 cru do Caddy.
...
- [ ] Mesclar na `main` publica sozinho **depois de o `CI` ficar verde**; uma
      imagem que não sobe é revertida sozinha e **o job fica vermelho**. Um
      commit com teste Python quebrado **não** constrói nada.
- [ ] O backup roda por `cron` **do root**, e **uma restauração foi feita e
      conferida**. Confirmado na manhã seguinte pelo log, não pelo silêncio.
